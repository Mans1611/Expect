...
const Minute = ({matchTime,min,setMin,halfsTime})=> {
    ...
    return (
    <div className={`circle circleAdm ${store.match.matchStatue === "UpComing" ? 'noCircle' : ''}`}>
        <span className="time">
            {
                (store.match.matchStatue !== "GoingOn") ? store.match.matchStatue === "UpComing" && <div className="upComing">Up-Coming</div>   :  min 
            }
